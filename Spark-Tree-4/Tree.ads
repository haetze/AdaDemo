...
is
   ...
   with 
     Pre => N.C < Count'Last and (for all I in A'Range => Contained(N, A(I))),
     Post => N.C'Old + 1 = N.C and 
	     ...
	     Contained(N, E) and
	     (for all I in A'Range => Contained(N, A(I)));
   
   
   function Insert(A : in Arr) return Node_P
   with
     Pre => A'Length <= Count'Last,
     Post => (if Insert'Result /= null then Insert'Result.C = A'Length) and
	     (for all I in A'Range => Contained(Insert'Result, A(I)));
